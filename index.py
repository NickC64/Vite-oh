...
class Bot(commands.Bot):
    def __init__(self):
        super().__init__(
            command_prefix="/", intents=intents, tree_cls=CommandTree
        )
...
class ProposalView(discord.ui.View):
    ...
    async def veto_button(
        ...
        await interaction.response.send_message(
            "Check your DMs for the veto confirmation.", ephemeral=True
        )
...
async def new(interaction: discord.Interaction, name: str):
    ...
    response_message = (
        f"A member proposal for {name} was added, set to pass <t:{deadline}:R>"
    )
    ...
    if output_channel:
        proposal_message = await output_channel.send(
            response_message, view=ProposalView(proposal_id)
        )
        proposals[proposal_id][
            "message_id"
        ] = proposal_message.id  # Store the message ID
...
async def notify_subscribers(proposal, status):
    for user_id in proposal["subscribers"]:
        ...
        if user:
            await user.send(
                f"The proposal for {proposal['name']} has been {status}."
            )
